...
processed_data = processed_data.fillna(
    {"Vehicle_brand": "", "Vehicle_model": "", "Vehicle_generation": ""}
)
raw_data = raw_data.fillna(
    {"Vehicle_model": "", "Vehicle_version": "", "Features": "[]"}
)
...
def brand_information() -> List[VehicleBrand]:
    ...
    brand_model_dict = (
        processed_data.groupby("Vehicle_brand")["Vehicle_model"].unique().to_dict()
    )

    # All generations for model
    model_generation_dict = (
        processed_data.groupby("Vehicle_model")["Vehicle_generation"]
        .unique()
        .apply(list)
        .to_dict()
    )
    ...
    model_version_dict = (
        raw_data.groupby("Vehicle_model")["Vehicle_version"]
        .unique()
        .apply(list)
        .to_dict()
    )
    ...
    for brand in brands:
        ...
        result.append(VehicleBrand(name=brand, vehicle_model=vehicle_models))
...
def dataset_values() -> ResultJSON:
    ...
    features = list(
        set(
            [
                feature
                for sublist in raw_data["Features"].dropna().apply(eval)
